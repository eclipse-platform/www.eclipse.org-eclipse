...
- `Cursor(Device, ImageData, ImageData, int, int)`: This constructor that accepts a single ImageData object (containing both the image and mask information) is not deprecated since the mask data is directly encoded into the image and as a user you don't need to define mask pixel by pixel.
